...
import torch
from botorch.acquisition.objective import MCAcquisitionObjective #// not used
...
logger: Logger = get_logger('MORBO_TrustRegion')
...
class TurboHParams:
    ...
    def from_dict(cls, tr_hparams: Dict) -> None:
        ...
        if unexpected_keys:
            logger.warning(f"Got unexpected tr_hparams keys, ignoring: {unexpected_keys}")
        ...
        with torch.no_grad():
            ...
            for x_ in X.split(self.tr_hparams.pred_batch_limit):
                ...
                if sampler is not None:
                    ...
                    pm = posterior.mean
                    predictions.append(pm)#pm.squeeze(-2) if gp_flag else pm)
                    continue
    ...
    def _has_improved_objective(self, n_new: int) -> bool:
        ...
        if obj.shape[0] > 0:
            ...
            if pareto_mask[-(obj.shape[0]) :].any():
                ...
                if new_hv > (1 + self.tr_hparams.eps) * self.hv and self.tr_hparams.verbose:
