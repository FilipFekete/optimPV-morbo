...
class CASMOPOLITANMixed(CASMOPOLITANCat):
    """Mixed categorical-continuous CASMOPOLITAN optimizer state.
    ...
    *Note*: in general, you should have the first d_cat dimensions as cat_dims, and the rest as the continuous dims.
    ...
    def __init__(
        ...
        """Initialize the mixed CASMOPOLITAN optimizer state.
        ...
        lb : numpy.ndarray
            Lower bounds for continuous dimensions.
        ub : numpy.ndarray
            Upper bounds for continuous dimensions.
        n_init : int
        ...
        int_constrained_dims : list[int] | None, optional
            Continuous dimensions constrained to integer values.
    ...
    def _adjust_length(self, fX_next):
        """Update mixed trust-region lengths from the newest objective values.
    ...
    def _create_and_select_candidates(self, X, fX, length, n_training_steps, hypers, return_acq=False):
        """Fit a local mixed GP and generate the next candidate batch.

        Parameters
        ----------
        X : numpy.ndarray
            Observed points in mixed representation.
        fX : numpy.ndarray
            Standardized objective values for ``X``.
        length : float | int
            Current categorical TR radius.
        n_training_steps : int
            Number of GP training steps.
        hypers : dict
            Optional GP state used to warm-start hyperparameters.
        return_acq : bool, optional
            Whether to also return acquisition values for the selected
            candidates.

        Returns
        -------
        numpy.ndarray | tuple[numpy.ndarray, numpy.ndarray]
            Selected candidates, optionally together with their acquisition values.
        """
