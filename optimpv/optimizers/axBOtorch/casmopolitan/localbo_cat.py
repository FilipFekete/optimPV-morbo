...
class CASMOPOLITANCat:
    """Categorical CASMOPOLITAN optimizer state.

    Parameters
    ----------
    dim : int
        Number of dimensions in categorical space.
    n_init : int
        Number of initial points (2*dim is recommended), ignored when initial points are provided.
    max_evals : int
        Total evaluation budget.
    config : array-like
        Configuration of the categorical variables.
    batch_size : int, optional
        Number of points in each batch.
    verbose : bool, optional
        If you want to print information about the optimization progress.
    use_ard : bool, optional
        If you want to use ARD for the GP kernel.
    max_cholesky_size : int, optional
        Largest number of training points where we use Cholesky.
    n_training_steps : int, optional
        Number of training steps for learning the GP hypers.
    min_cuda : int, optional
        We use float64 on the CPU if we have this or fewer datapoints.
    device : str, optional
        Device to use for GP fitting ("cpu" or "cuda")
    dtype : str, optional
        Dtype to use for GP fitting ("float32" or "float64")
    acq : str, optional
        Acquisition function identifier.
    kernel_type : str, optional
        Kernel variant used for the GP.
    **kwargs
    ...
    def __init__(
            ...
            n_training_steps=50, # was hardcoded to 300 in the original
            min_cuda=1024,
            device="cpu",
            dtype="float32",
            acq='thompson',
        ...
        """Initialize the categorical CASMOPOLITAN optimizer state.

        Parameters
        ----------
        dim : int
            Problem dimensionality in ordinal categorical space.
        n_init : int
            Number of initial design points.
        max_evals : int
            Maximum evaluation budget tracked by the optimizer.
        config : array-like
            Number of categories for each categorical variable.
        batch_size : int, optional
            Number of candidates generated per batch.
        verbose : bool, optional
            Whether to print progress information.
        use_ard : bool, optional
            Whether ARD is enabled for the GP kernel.
        max_cholesky_size : int, optional
            Largest training set size for Cholesky-based GP operations.
        n_training_steps : int, optional
            Number of GP hyperparameter training steps.
        min_cuda : int, optional
            Threshold below which fitting is forced onto CPU.
        device : str, optional
            Requested GP device.
        dtype : str, optional
        ...
        acq : str, optional
            Acquisition function identifier.
        kernel_type : str, optional
            Kernel identifier used by the GP.
        **kwargs
            Additional optimizer options.
        """
    ...
    def _restart(self):
        """Restart local optimization state."""
        self._X = []
        self._fX = []
        self.failcount = 0
        self.succcount = 0
    ...
    def _adjust_length(self, fX_next):
        """Update TR lengths from the newest objective values.

        Parameters
        ----------
        fX_next : numpy.ndarray
            Newly observed objective values for the pending batch.
        """
    ...
    def _create_and_select_candidates(self, X, fX, length, n_training_steps, hypers, return_acq=False):
        """Fit a local GP and generate the next candidate batch.
        ...
        X : numpy.ndarray
            Observed points.
        fX : numpy.ndarray
            Standardized objective values for ``X``.
        length : float | int
            Current TR radius in categorical space.
        n_training_steps : int
            Number of GP training steps.
        hypers : dict
            Optional GP state used to warm-start hyperparameters.
        return_acq : bool, optional
            Whether to also return acquisition values for the selected candidates.

        Returns
        -------
        numpy.ndarray | tuple[numpy.ndarray, numpy.ndarray]
            Selected candidates, optionally together with their acquisition
            values.
        """
