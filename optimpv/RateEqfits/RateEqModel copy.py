...
def DBTD_model(parameters, t, Gpulse, t_span, N0=0, G_frac = 1, equilibrate=True, eq_limit=1e-2, maxcount=1e3, output_integrated_values = True,**kwargs):
    ...
    if output_integrated_values:
        # Rrad_calc = (n_dens * p_dens) #* k_rad
        # PL_calc = np.trapezoid(n_dens, z_array)*np.trapezoid(p_dens, z_array) * k_rad
        # PL_calc = np.sum((Rrad_calc[:, 1:] + Rrad_calc[:, :-1]) / 2, axis=1) * ds
        # n_dens = np.sqrt(PL_calc)
        # p_dens = np.sqrt(PL_calc)
        # integrate the densities over the spatial domain
        # n_dens = np.trapezoid((n_dens[:, 1:]+ n_dens[:, :-1])/2, z_array[1:], axis=1)
        # p_dens = np.trapezoid((p_dens[:, 1:]+ p_dens[:, :-1])/2, z_array[1:], axis=1)
...
def DBTD_multi_trap(parameters, t, Gpulse, t_span, N0=0, G_frac = 1, equilibrate=True, eq_limit=1e-2, maxcount=1e3, output_integrated_values = True,**kwargs):
    ...
    # traps 
    trapsnames = [p.name for p in parameters if 'N_t_bulk' in p.name]
    Cnnames = [p.name for p in parameters if 'C_n' in p.name]
    Cpnames = [p.name for p in parameters if 'C_p' in p.name]
    Etrapnames = [p.name for p in parameters if 'E_trap' in p.name]
    ...
    N_traps = len(N_t_bulk_list)
    grid_size = kwargs.get('grid_size', 100)  # number of grid points
    z_array = np.linspace(0, L, grid_size)  # spatial domain
    N_x = grid_size
    dx = z_array[1] - z_array[0]  # spatial step size
    D_d = [D_n, D_p]
    ...
    def model_vect(t, P_flat, kdirect, Eg, Bulk_tr, Bn, Bp, ETrap, Nc, Nv, T):
        if P_flat.ndim == 1:
            P_flat = P_flat[:, None]  # make it (n_variables, 1)

        n_times = P_flat.shape[1]
        
        P = P_flat.reshape(N_traps + 2, N_x, n_times)  # Now (populations, space, n_times)
        ...
        # Diffusion terms
        d2n = np.array([second_derivative(n[:, i], N_x, dx) for i in range(n_times)]).T
        d2p = np.array([second_derivative(p[:, i], N_x, dx) for i in range(n_times)]).T

        dPdt = np.zeros_like(P)

        dPdt[0] = - kdirect * (n * p - ni2) - np.sum(e_capture, axis=0) + np.sum(e_emission, axis=0) + D_d[0] * d2n
        dPdt[1] = - kdirect * (n * p - ni2) - np.sum(h_capture, axis=0) + np.sum(h_emission, axis=0) + D_d[1] * d2p
    ...
    def jacobian_no_flux_vectorized_fixed(t, P_flat, *args):
        ...
        idx_n = np.arange(N_x)
        idx_p = N_x + np.arange(N_x)
        idx_traps = [N_x * (2 + j) + np.arange(N_x) for j in range(N_traps)]
        ...
        # Diffusion for n
        diag_n[1:-1] += -2 * D_d[0] / dx**2
        J[idx_n[1:-1], idx_n[:-2]] = D_d[0] / dx**2
        J[idx_n[1:-1], idx_n[2:]]  = D_d[0] / dx**2
        diag_n[0] += -1 * D_d[0] / dx**2
        J[idx_n[0], idx_n[1]] = D_d[0] / dx**2
        diag_n[-1] += -1 * D_d[0] / dx**2
        J[idx_n[-1], idx_n[-2]] = D_d[0] / dx**2

        # Diffusion for p
        diag_p[1:-1] += -2 * D_d[1] / dx**2
        J[idx_p[1:-1], idx_p[:-2]] = D_d[1] / dx**2
        J[idx_p[1:-1], idx_p[2:]]  = D_d[1] / dx**2
        diag_p[0] += -1 * D_d[1] / dx**2
        J[idx_p[0], idx_p[1]] = D_d[1] / dx**2
        diag_p[-1] += -1 * D_d[1] / dx**2
        J[idx_p[-1], idx_p[-2]] = D_d[1] / dx**2
    ...
    def model_vect_dimensionless(t, P_flat_d, kdirect, Eg, Bulk_tr_d, Bn, Bp, ETrap, Nc_d, Nv_d, T):
        ...
        P = P_flat_d.reshape(N_traps + 2, N_x, n_times)  # Now (populations, space, n_times)
        ...
        # Diffusion terms
        d2n = np.array([second_derivative(n_d[:, i], N_x, dx) for i in range(n_times)]).T
        d2p = np.array([second_derivative(p_d[:, i], N_x, dx) for i in range(n_times)]).T

        dPdt = np.zeros_like(P)

        dPdt[0] = - kdirect * (n_d * p_d - 1) - np.sum(e_capture, axis=0) + np.sum(e_emission, axis=0) + D_d[0] * d2n
        dPdt[1] = - kdirect * (n_d * p_d - 1) - np.sum(h_capture, axis=0) + np.sum(h_emission, axis=0) + D_d[1] * d2p
    ...
    P0 = P_init.flatten()
    arg = [k_direct, Eg, N_t_bulk_list, C_n_bulk_list, C_p_bulk_list, E_t_bulk_list, N_c, N_v, T]
    
    nondimension = True
    RelChange = 1e40 # artificially large to start with
    if nondimension:
        n0_z = n0_z / ni  # non-dimensionalize the initial charge carrier density
        P0 = P0 / ni
        x = np.linspace(0, 1, grid_size)
        dx = x[1] - x[0]
        tau = np.average(1/(N_t_bulk_list * np.sqrt(C_n_bulk_list * C_p_bulk_list)))  # non-dimensionalize the time
        t_span = t_span / tau
        arg = [k_direct * ni * tau, Eg, N_t_bulk_list/ni, C_n_bulk_list * ni * tau, C_p_bulk_list * ni * tau, E_t_bulk_list, N_c/ni, N_v/ni, T]
        D_n = D_n * tau/(L**2)
        D_p = D_p * tau/(L**2)
    ...
    t_start = time.time()
    count = 0
    end_point = P0[:N_x] # initial end point for convergence check
    Ngen = n0_z # generation term

    if equilibrate:
        while np.any(abs(RelChange) > eq_limit) and count < maxcount:
            if time.time() - t_start > timeout:
                warnings.warn("Equilibration took too long, stopping.")
                return np.nan * np.ones((grid_size, len(t))), np.nan * np.ones((grid_size, len(t)))
            
            if (nondimension):
                sol_single = solve_ivp(model_vect_dimensionless, [t_span[0], t_span[-1]], P0, method='BDF', args=arg, vectorized=True, jac=jacobian_no_flux_vectorized_fixed, rtol=1e-3, atol=1e-3)
            else:
                sol_single = solve_ivp(model_vect, [t_span[0], t_span[-1]], P0, method='BDF', args=arg, vectorized=True, jac=jacobian_no_flux_vectorized_fixed, rtol=1e-3, atol=1e-3)
        
            if not(sol_single.success):
                print('Integration failed:', sol_single.status, sol_single.message)                            
                return np.nan * np.ones((grid_size, len(t))), np.nan * np.ones((grid_size, len(t)))
            
            sol_flat = sol_single.y.reshape(len(P_init), N_x, -1)
            n_last = sol_flat[0, :, -1]
            p_last = sol_flat[1, :, -1]
    ...
    # Final solve with actual time points t
    t_normalized = t if not nondimension else t / tau
    sol = solve_ivp(model_vect_dimensionless if nondimension else model_vect, 
                    [t_normalized[0], t_normalized[-1]], P0, 
                    t_eval=t_normalized, method='BDF', args=arg, 
                    vectorized=True, jac=jacobian_no_flux_vectorized_fixed, 
                    rtol=1e-3, atol=1e-3)

    if not sol.success:
        print('Final integration failed:', sol.status, sol.message)
        return np.nan * np.ones(len(t)), np.nan * np.ones(len(t))

    # Reshape solution
    sol_reshaped = sol.y.reshape(len(P_init), N_x, -1)
    n_solution = sol_reshaped[0] * (ni if nondimension else 1)  # re-dimensionalize
    p_solution = sol_reshaped[1] * (ni if nondimension else 1)  # re-dimensionalize

    if output_integrated_values:
        # Integrate over spatial domain
        n_integrated = np.trapz(n_solution, z_array, axis=0)
        p_integrated = np.trapz(p_solution, z_array, axis=0)
        return n_integrated, p_integrated
    else:
        return n_solution, p_solution
